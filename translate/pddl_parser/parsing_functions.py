                ...
                try:
                    ...
                    if not (isinstance(_type, str) or
                            (_type and _type[0] == "either" and
                             all(isinstance(_sub_type, str) for _sub_type in
                                 _type[1:]))):
                        context.error(
                            "Type value is expected to be a single word "
                            "or '(either WORD*)")
                for item in items:
...
def parse_condition_aux(context, alist, negated, type_dict, predicate_dict):
    ...
    if tag in ("and", "or", "not", "imply"):
        ...
        return parse_literal(context, alist, type_dict, predicate_dict,
                             negated=negated)
    else:
    ...
    if tag == "imply":
        ...
        parts = [parse_condition_aux(context, part, negated, type_dict,
                                     predicate_dict)
                 for part in args]
...
def add_outcomes(tmp_effect, result):
    ...
    if isinstance(tmp_effect, pddl.ProbabilisticEffect):
        for prob, effect in tmp_effect.effect_probability_pairs:
            effects = []
...
def add_effects(tmp_effect, result):
    """tmp_effect has the following structure:
       [ConjunctiveEffect] [UniversalEffect]
       [ConditionalEffect] SimpleEffect."""

    if isinstance(tmp_effect, pddl.ConjunctiveEffect):
        for effect in tmp_effect.effects:
            add_effects(effect, result)
        return
    else:
        parameters = []
...
def parse_effect(context, alist, type_dict, predicate_dict, has_reward_fluent):
    ...
    if tag == "and":
        ...
        if remaining_probability < 0:
            context.error(
                "Total probability is higher than one",
                alist,
                syntax=SYNTAX_EFFECT_PROBABILISTIC)

        if remaining_probability > 0:
            outcomes.append((remaining_probability,
        ...
        return pddl.SimpleEffect(
            parse_literal(context, alist, {}, predicate_dict))
...
def parse_action(context, alist, type_dict, predicate_dict, has_reward_fluent):
    with context.layer("Parsing action name"):
        ...
        name = next(iterator)
        if not isinstance(name, str):
            context.expected_word_error("Action name", name,
                                        syntax=SYNTAX_ACTION)
        ...
        try:
            ...
            with context.layer("Parsing precondition"):
                if precondition_tag_opt == ":precondition":
                    ...
                    if not isinstance(precondition_list, list):
                        context.expected_list_error(
                            "Precondition", precondition_list,
                            syntax=SYNTAX_ACTION)
                    if not precondition_list:
                        ...
                        precondition = parse_condition(
                            context, precondition_list, type_dict,
                            predicate_dict)
            ...
            with context.layer("Parsing effect"):
                ...
                if not isinstance(effect_list, list):
                    context.expected_list_error("Effect", effect_list,
                                                syntax=SYNTAX_ACTION)
                outcomes = []
                if effect_list:
                    cost = parse_outcomes(
...
def parse_axiom(context, alist, type_dict, predicate_dict):
    ...
    with context.layer(
            f"Parsing condition for derived predicate '{predicate}'"):
        if not isinstance(alist[2], list):
            context.error(
                "The second argument (CONDITION) is expected to be a block.",
                syntax=SYNTAX_AXIOM)
        ...
        with context.layer(f"Parsing {no}. element in init block"):
            ...
            if fact[0] == "=":
                ...
                if not isinstance(assignment.expression,
                    ...
                    context.error(
                        "Illegal assignment in initial state specification.",
                        assignment)
                ...
                if len(fact) != 2:
                    context.error(
                        f"Expecting {SYNTAX_LITERAL_NEGATED} for negated atoms.")
                fact = fact[1]
                if not isinstance(fact, list) or not fact:
                    context.error("Invalid negated fact.",
...
def parse_domain_pddl(context, domain_pddl):
    ...
    with context.layer("Parsing domain"):
        ...
        if define_tag != "define":
            context.error(
                f"Domain definition expected to start with '(define '. Got '({define_tag}'")

        with context.layer("Parsing domain name"):
            domain_line = next(iterator)
            if (not check_named_block(domain_line, ["domain"]) or
                    len(domain_line) != 2 or not isinstance(domain_line[1],
                                                            str)):
        ...
        first_action = None
        for opt in iterator:
            assert_named_block(context, opt,
                               correct_order + action_or_axiom_block)
            field = opt[0]
            ...
            if (seen_fields and
                    correct_order.index(seen_fields[-1]) > correct_order.index(
                        field)):
    ...
    with context.layer("Parsing task"):
        ...
        with context.layer("Parsing problem name"):
            ...
            if (not check_named_block(problem_line, ["problem"]) or
                    len(problem_line) != 2 or not isinstance(problem_line[1],
                                                             str)):
        ...
        assert_named_block(context, requirements_opt,
    ...
    if atom in initial_proposition_values:
        ...
        if prev_value != atom_value:
            ...
            if atom_value is False:
                atom = atom.negate()
            print(
                f"Warning: {atom} is specified twice in initial state specification")
